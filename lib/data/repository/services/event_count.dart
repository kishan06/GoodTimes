...
class EventCountReportServices{
  Dio dio = Dio();
    GlobalController globalController = getxx.Get.find();
  Future<ResponseModel> eventCountReportServices(context,{id}) async {
    try {
      final header = {
        "Content-Type": "application/json",
        "Authorization": "Bearer ${GetStorage().read('accessToken')}"
      };
      Response response = await dio.post("${Endpoints.eventReportCount}$id/view/",options: Options(headers: header));
