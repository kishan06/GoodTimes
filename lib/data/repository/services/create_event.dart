...
class CreateEventService {
  ...
  Future<ResponseModel> createEventServices(
    context, {
    title,
    description,
    ageGroup,
    ...
    logger.e(
        'Event title in services $title - descriptions $description - start date $startDate - start time $fromTime - end data $endDate - end time $toTime - select venu $venue - age group $ageGroup - category $category - venu capacity $venueCapacity - entry type ${entryType.toLowerCase()} - admission cost $enteryFee - key guest $guest - thumbnail image $thumbnailImg - photos ab $images tags $tags tags ${jsonEncode(tags)} coupon code $couponCodeController coupon code descriptions $couponDescriptionController');
...
// Edit events service function start from here
  Future<ResponseModel> editEventServices(context,
      ...
      draft}) async {
    logger.e(
        'Edit event title in services $eventId $title - descriptions $description - start date $startDate - start time $fromTime - end data $endDate - end time $toTime - select venu $venue - age group $ageGroup - category $category - venu capacity $venueCapacity - entry type $entryType - admission cost $enteryFee - key guest $guest - thumbnail image $thumbnailImg - photos ab $images  $tags tags ${jsonEncode(tags)}');
    ...
    try {
      ...
      logger.e(
          'respose of create event exceptions ${e.response?.data["message"]}');
      if (e.response?.statusCode == 400) {
        // Handle 400 Bad Request Error
        logger.e('Bad Request Error: ${e.message}');
